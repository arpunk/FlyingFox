defmodule FlyingFox.TxUpdate do
  ...
  def wait2bond(tx, d) do
    ...
    b = FlyingFox.KV.get("tot_bonds")
    FlyingFox.KV.put("tot_bonds", b + (a * exchange_rate * d))
    ...
    #participate. If you participate, then the value of your
    #bond-money will slowly grow. If you dont participate, then the
    #value will quickly shrink.  There is a minimum size for
    #purchasing bond-money, priced in spend-money.
  end

  def bond2spend(tx, d) do
    a = tx[:data][:amount]
    b = FlyingFox.KV.get("tot_bonds")
    FlyingFox.KV.put("tot_bonds", b - (a * d))
  ...
  def sign(tx, d, bond_size) do
    # 0.1% of total bonds is given out as rewards on every block, which changes the exchange rate.
    w = length(tx[:data][:winners])
    delta = -exchange_rate * bond_size * w
    b = FlyingFox.KV.get("tot_bonds")
    FlyingFox..KV.put("tot_bonds", b + delta * d)
  ...
  def common(tx, d, old_block, signer) do
    ...
    reward = FlyingFox.KV.get("tot_bonds") / :math.pow(1.001, @epoch) * w / 1000 / @signers_per_block
    sym_append(hd(FlyingFox.KV.get(to_string(tx[:data][:signed_on]))), [:meta, :revealed], signer, d)
    {reward, delta}
  end

  def slasher(tx, d) do
    # If you can prove that the same address signed on 2 different blocks at the same height,
    # then you can take 1/3rd of the deposit, and destroy the rest.
    a = tx[:data]
    old_block = FlyingFox.Blockchain.get_block(a[:tx1][:data][:height])
    {reward, delta} = common(tx, d, old_block, a[:tx1][:pub])
    sym_increment(tx[:pub], :amount, tx[:data][:amount] + reward + delta / 3, d)
  end

  def reveal(tx, d) do
    old_block = FlyingFox.Blockchain.get_block(tx[:data][:signed_on])
    {reward, delta} = common(tx, d, old_block, tx[:pub])
    # During waiting period you are holding cash not bonds.
    sym_increment(tx[:pub], :amount, tx[:data][:amount] + reward + delta, d)
  end

	def to_channel(tx, d) do
    sym_increment(tx[:pub], :amount, -tx[:data][:amount] - tx[:data][:fee], d)
    cond do
      tx[:new] and d == 1 -> FlyingFox.KV.put(tx[:channel], [pub: tx[:pub],
                                                             pub2: tx[:data][:pub2],
                                                             ...
                                                             nonce: 0,
                                                             delay: tx[:delay]])
      tx[:new] -> FlyingFox.KV.put(tx[:channel], nil)
      true -> sym_increment(tx[:channel], tx[:to], tx[:data][:amount], d)
    end
	end

	def channel_block(tx, d) do
    # Need to make a timestamp now for possible refund tx. Also, we should record the
    # nonce for this channel state, so we only update one way.
    sym_replace(tx[:channel], :time, 0, FlyingFox.KV.get("height"), d)
