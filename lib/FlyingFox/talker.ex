defmodule FlyingFox.Talker do
  ...
  def add_peers(x) do
    Enum.map(x, fn(x) -> FlyingFox.Peers.add_peer(x) end)
  end
  ...
  def download(n, i, p, out \\ []) do
    ...
    cond do
      lo >= n -> out
      true ->
        out = out ++ FlyingFox.Api.blocks(i + lo, i + n, p[:port], p[:ip])
  ...
  def download_blocks(i, u, p) do
    ...
    my_block = FlyingFox.Api.blocks(i, i)

    cond do
      my_block == [] ->
        FlyingFox.BlockAbsorber.absorb(blocks)
        [status: :first_blocks]
  ...
  def trade_peers(p) do
    my_peers = FlyingFox.Api.all_peers
    peers = FlyingFox.Api.all_peers(p[:port], p[:ip])
    if my_peers == :ok or peers == :ok do
      IO.puts("peer died 1")
    else
      not_yours = Enum.filter(my_peers, &(not &1 in peers))
      not_mine = Enum.filter(peers, &(not &1 in my_peers))
      Enum.map(not_yours, &(FlyingFox.Api.add_peer(elem(&1, 1), p[:port], p[:ip])))
      Enum.map(not_mine, &(FlyingFox.Peers.add_peer(elem(&1, 1))))
    end
  end

  def check_peer(p) do
    #validating mode
    status = FlyingFox.Api.status(p[:port], p[:ip])
    cond do
  ...
  def check_peer_2(p, status) do
    trade_peers(p)
    txs = FlyingFox.Api.txs(p[:port], p[:ip])
    u = status[:height]
    i = FlyingFox.KV.get("height")
    cond do
      txs == :ok -> IO.puts("txs shouldn't be :ok")
      (txs != []) and length(txs) > 0 and is_tuple(hd(txs)) -> IO.puts("tx error #{inspect txs}")
      u > i -> download_blocks(i, u, p)
      u == i -> Enum.map(txs, &(FlyingFox.Mempool.add_tx(&1)))
      true ->
        IO.puts("im ahead")
        true
    end
  ...
  def init(args) do
    start
    Enum.map(0..2, &([ip: "localhost", port: @tcp_port + &1]))
    |> Enum.map(&(FlyingFox.Peers.add_peer(&1)))
    {:ok, args}
  end
